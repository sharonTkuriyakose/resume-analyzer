...
const ResultPage = ({ data }) => {
  ...
  React.useEffect(() => {
    if (data && Object.keys(data).length > 0) {
      ...
      console.log("%c[NEURAL LAB] RAW DATA EXTRACTED (FOR RESEARCH)", "color: #10b981; font-weight: bold; font-size: 16px;");
      console.log("%c===========================================", "color: #10b981; font-weight: bold; font-size: 16px;");
  ...
  if (!data || Object.keys(data).length === 0) {
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center text-white bg-[#050505]">
        <Activity className="w-12 h-12 text-[#10b981] animate-pulse mb-4" />
  ...
  const NodeButton = ({ id, icon: Icon, title, onClick, positionClass }) => (
    <motion.button 
      ...
      whileTap={{ scale: 0.95 }}
      className={`xl:absolute ${positionClass} w-full xl:w-[300px] xl:h-[100px] glass-card rounded-2xl z-10 flex items-center justify-start px-6 py-4 xl:py-0 gap-4 group cursor-pointer hover:border-[#10b981]/50 hover:bg-white/[0.05] hover:shadow-[0_0_20px_rgba(16,185,129,0.3)] transition-all`}
    >
      <div className="w-10 h-10 xl:w-12 xl:h-12 rounded-full bg-black/50 border border-white/10 flex items-center justify-center shrink-0 group-hover:border-[#10b981]/50 group-hover:bg-[#10b981]/10 transition-colors">
        <Icon className="w-5 h-5 text-[#10b981]" />
      </div>
      <div className="flex flex-col items-start text-left">
        <h3 className="text-[11px] xl:text-[13px] font-black text-white uppercase tracking-widest">{title}</h3>
        <span className="text-[9px] xl:text-[10px] text-[#10b981] uppercase tracking-widest mt-1 xl:opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
        ...
        <div className="w-full flex flex-col md:flex-row items-center justify-between mb-8 gap-6 bg-[#111]/80 backdrop-blur-xl border border-white/10 rounded-2xl p-4 md:px-8 shadow-2xl relative overflow-hidden print:hidden">
           ...
           <div className="flex flex-col items-center md:items-start">
             <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1 flex items-center gap-2">
                <Target className="w-3 h-3 text-[#10b981]" /> Neural Lab / Analysis Complete
             </div>
             <div className="flex items-center gap-3">
               <h1 className="text-xl md:text-2xl font-black text-white uppercase tracking-tight">Candidate Profile</h1>
               <span className="px-3 py-1 bg-white/5 border border-white/10 rounded-full text-[10px] font-bold text-[#10b981] uppercase tracking-wider">
        ...
        <div className="w-full print:hidden">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="absolute inset-0 pointer-events-none hidden xl:block z-0">
                 <svg viewBox="0 0 1600 1000" className="w-full h-full overflow-visible">
                    ...
                    {/* Lines */}
                    <motion.g fill="none" stroke="#10b981" strokeWidth="2" filter="url(#glow)" className="opacity-60">
                      {/* Top Left to Center */}
              ...
              <div className="xl:absolute xl:top-1/2 xl:left-1/2 xl:-translate-x-1/2 xl:-translate-y-1/2 w-[240px] h-[240px] sm:w-[300px] sm:h-[300px] xl:w-[350px] xl:h-[350px] z-20 flex items-center justify-center relative mb-4 xl:mb-0 shrink-0 mx-auto">
                 <div className="absolute inset-0 bg-[#10b981]/20 rounded-full blur-[60px] animate-pulse"></div>
                 <div className="relative w-full h-full bg-[#111]/80 backdrop-blur-xl rounded-full border border-white/10 flex flex-col items-center justify-center shadow-[inset_0_0_60px_rgba(16,185,129,0.2),0_0_40px_rgba(0,0,0,0.8)]">
                    <svg className="absolute w-[90%] h-[90%] transform -rotate-90 drop-shadow-[0_0_15px_rgba(16,185,129,0.5)]" viewBox="0 0 120 120">
                      <circle cx="60" cy="60" r="54" stroke="currentColor" strokeWidth="3" fill="transparent" className="text-white/5" />
                      <motion.circle 
                        cx="60" cy="60" r="54" 
                        stroke="url(#greenGradient)" strokeWidth="6" fill="transparent" strokeLinecap="round" 
                    ...
                    <div className="z-10 flex flex-col items-center mt-4">
                       ...
                       <div className="text-5xl sm:text-7xl font-black text-white drop-shadow-lg flex items-start">
                         {data.score || 0}<span className="text-xl sm:text-3xl text-[#10b981] mt-1 sm:mt-2">%</span>
                       </div>
                       <div className="mt-2 px-3 py-1 border border-[#10b981]/30 bg-[#10b981]/10 rounded-full text-[8px] sm:text-[10px] font-bold text-[#10b981] tracking-widest uppercase">
            ...
            <motion.div 
              ...
              className="w-full max-w-[1200px] mx-auto mt-12 bg-[#111]/80 backdrop-blur-xl border border-white/10 rounded-2xl p-6 md:p-10 mb-20 shadow-[0_10px_40px_rgba(0,0,0,0.5)] relative overflow-hidden"
            >
              <div className="absolute top-0 left-0 w-1 bg-gradient-to-b from-[#10b981] to-[#047857] h-full"></div>
              
              <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8 mb-8">
                <div>
                  <h2 className="text-xl font-black text-white uppercase tracking-widest flex items-center gap-3">
                    <Activity className="w-6 h-6 text-[#10b981]" /> Deductive Scoring Algorithm
                  </h2>
                ...
                <div className="text-sm text-emerald-400 font-mono bg-black/80 px-6 py-4 rounded-xl border border-emerald-500/20 shadow-inner">
              ...
              <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
                {[
                  ...
                  <div key={idx} className="bg-black/40 border border-white/5 rounded-xl p-4 flex flex-col items-center justify-center text-center relative group hover:border-emerald-500/30 transition-colors">
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">{comp.label}</span>
                    <div className="text-2xl font-black text-white my-2">{comp.val}<span className="text-sm text-emerald-500">%</span></div>
                    <span className="text-[9px] font-black text-emerald-500 bg-emerald-500/10 px-2 py-1 rounded-full uppercase tracking-wider">Weight: {comp.weight}</span>
            ...
            >
              <button 
                ...
                className="absolute top-8 left-8 flex items-center gap-2 px-4 py-2 bg-black/50 hover:bg-[#10b981]/20 border border-white/10 hover:border-[#10b981]/50 rounded-lg text-xs font-bold uppercase tracking-widest text-slate-300 hover:text-white transition-all"
              >
              ...
              <div className="mt-16 w-full">
                ...
                {activeCard === 'ats' && (
                  <div className="w-full">
                     <h2 className="text-2xl font-black text-white uppercase tracking-tight flex items-center gap-3 mb-10">
                       <Search className="w-8 h-8 text-[#10b981]" /> ATS Intelligence Breakdown
                     </h2>
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                       ...
                       <div className="bg-black/40 border border-emerald-500/20 rounded-2xl p-8">
                          <h3 className="text-sm font-black text-emerald-500 uppercase tracking-widest mb-6 flex items-center gap-2">
                          ...
                          <div className="flex flex-col gap-3">
                            {data.keywordsMissing?.map((kw, i) => {
                              ...
                              const context = typeof kw === 'object' ? kw.context : null;
                              return (
                              <div key={i} className="p-3 bg-[#050505] border border-emerald-500/30 rounded-lg">
                                <span className="text-emerald-400 text-xs font-bold uppercase tracking-wider">{keyword}</span>
                                {context && <p className="text-[10px] text-slate-400 mt-2 leading-relaxed">{context}</p>}
                              </div>
                              );
                            })}
                ...
                {activeCard === 'skills' && (
                  <div className="w-full">
                     <h2 className="text-2xl font-black text-white uppercase tracking-tight flex items-center gap-3 mb-10">
                       <Zap className="w-8 h-8 text-[#10b981]" /> Skill Proficiency Matrix
                     </h2>
                ...
                {activeCard === 'projects' && (
                  <div className="w-full">
                     <h2 className="text-2xl font-black text-white uppercase tracking-tight flex items-center gap-3 mb-10">
                       <Sparkles className="w-8 h-8 text-[#10b981]" /> Strategic Projects Lab
                     </h2>
                     <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {projectStage?.projectList?.map((proj, i) => (
                          <div key={i} className="bg-black/50 border border-white/10 rounded-2xl p-6 hover:border-[#10b981]/50 transition-colors flex flex-col h-full">
                            <h4 className="text-sm font-black text-[#10b981] uppercase tracking-tight mb-2">{proj.name}</h4>
                            ...
                            <button className="mt-6 w-full py-3 bg-[#10b981]/10 hover:bg-[#10b981]/20 text-[#10b981] border border-[#10b981]/30 rounded-xl text-xs font-black uppercase tracking-widest transition-all">
                              Start Project
                            </button>
                          </div>
                        ))}
                ...
                {activeCard === 'roadmap' && (
                  <div className="w-full">
                     <h2 className="text-2xl font-black text-white uppercase tracking-tight flex items-center gap-3 mb-10">
                       <Briefcase className="w-8 h-8 text-[#10b981]" /> Career & Jobs Center
                     </h2>
                     <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
                       ...
                       <div>
                         <h3 className="text-sm font-black text-slate-300 uppercase tracking-widest mb-8 flex items-center gap-2">
                           <BookOpen className="w-4 h-4 text-[#10b981]" /> Phased Curriculum
                         </h3>
                         <div className="space-y-8 relative pl-4 border-l-2 border-white/10">
                            {data.phasedCurriculum?.filter(step => !step.isProject).map((item, i) => (
                              <div key={i} className="relative pl-6 group/item">
                                <div className="absolute left-[-29px] top-1 w-4 h-4 rounded-full bg-black border-2 border-[#10b981] group-hover/item:bg-[#10b981] transition-colors"></div>
                                <div className="text-[10px] font-black text-[#10b981] uppercase tracking-widest mb-1">Phase {item.id}</div>
