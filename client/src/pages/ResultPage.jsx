...
const ResultPage = ({ data }) => {
  ...
  // Extract the project-specific data from Phase 4
  ...
  return (
    <div className="min-h-screen w-full bg-[#0A0C10] text-white font-sans pt-6 pb-20 selection:bg-indigo-500/20">
      ...
      <div className="max-w-[1750px] mx-auto px-4 md:px-10 space-y-10 md:space-y-12 bg-[#0A0C10]">
        ...
        <main className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-10">
          ...
          {/* RIGHT COLUMN */}
          <section className="col-span-1 lg:col-span-8 space-y-12 overflow-hidden">
            
            {/* 1. ATS KEYWORD INTELLIGENCE */}
            <div className="p-8 md:p-12 rounded-[2.5rem] bg-white/[0.03] border border-white/10 text-left card-bg shadow-2xl">
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {data.keywordsDetected?.map((kw, i) => (
                      <div key={i} className="px-5 py-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex items-center gap-3">
                        <div className="w-2 rounded-full bg-emerald-400 aspect-square shadow-[0_0_8px_rgba(52,211,153,0.6)]" />
                        <span className="text-xs md:text-sm font-black text-white uppercase tracking-wider">{kw}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {data.keywordsMissing?.map((kw, i) => (
                      <div key={i} className="px-5 py-3 bg-amber-500/10 border border-amber-500/20 rounded-xl flex items-center gap-3">
                        <div className="w-2 rounded-full bg-amber-500 aspect-square shadow-[0_0_8px_rgba(245,158,11,0.6)]" />
                        <span className="text-xs md:text-sm font-black text-white uppercase tracking-wider">{kw}</span>
                      </div>
                    ))}
                  </div>
            ...
            {/* 2. PROJECT LAB - FIXED OVERFLOW & SPACING */}
            <div className="p-8 md:p-12 rounded-[2.5rem] bg-white/[0.02] border border-white/10 text-left card-bg shadow-2xl">
              ...
              {/* FIXED GRID: Ensures layout doesn't squeeze cards on medium screens */}
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {projectStage?.projectList?.map((proj, i) => (
                  <div key={i} className="p-6 md:p-8 rounded-[2.5rem] bg-white/5 border border-white/5 hover:border-amber-400/30 transition-all group flex flex-col h-full min-w-0">
                    <span className="text-[9px] md:text-[10px] font-black text-gray-500 uppercase tracking-widest mb-4 block opacity-60">Project Simulation</span>
                    
                    {/* FIXED TITLE: break-words prevents vertical clipping */}
                    <h4 className="text-white font-black text-lg md:text-xl mb-8 uppercase tracking-tight leading-[1.2] group-hover:text-amber-400 transition-colors break-words">
                      {proj.name}
                    </h4>
                    ...
                    <div className="space-y-6 mt-auto">
                      {proj.points?.map((point, idx) => (
                        <div key={idx} className="flex items-start gap-3">
                          ...
                          {/* FIXED LIST: break-words and smaller text for better fit */}
                          <p className="text-[10px] md:text-[11px] text-slate-400 font-bold uppercase tracking-tight leading-relaxed break-words">
                            {point}
                          </p>
                        </div>
                      ))}
              ...
              <div className="space-y-10 relative">
                ...
                {data.phasedCurriculum?.map((item, i) => (
                  <div key={i} className="relative pl-0 md:pl-20 group">
                    <div className={`absolute left-0 top-0 hidden md:flex w-12 h-12 rounded-full bg-[#0A0C10] border-2 items-center justify-center font-black z-10 transition-all group-hover:border-emerald-400 text-white
                      ${item.id === 2 ? 'border-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.3)]' : 'border-white/20'}`}>
                      {item.id}
                    </div>
                    <div className="p-8 rounded-[2.5rem] bg-white/5 border border-white/5 hover:border-white/20 transition-all">
                      <h4 className="text-xl md:text-2xl font-black text-white uppercase mb-4 tracking-tight break-words">{item.title}</h4>
                      <p className="text-[11px] md:text-[12px] text-slate-500 uppercase font-black tracking-widest mb-8 italic leading-relaxed">Objective: {item.primaryGoal}</p>
                      
                      {!item.isProject && (
                        <ul className="mb-8 space-y-3">
                           {item.points?.map((p, idx) => (
                             <li key={idx} className="text-[11px] md:text-[12px] text-gray-400 flex items-center gap-3">
                               <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-[0_0_5px_rgba(16,185,129,0.5)]" /> {p}
                             </li>
                           ))}
                        </ul>
                      )}
                      ...
                      <div className="flex flex-col sm:flex-row gap-4 pt-8 border-t border-white/5">
                        <a href={item.docLink} target="_blank" rel="noreferrer" className="w-full sm:w-auto flex-1 px-6 py-4 bg-white/5 border border-white/10 rounded-xl text-[10px] font-black uppercase text-white text-center flex items-center justify-center gap-2 hover:bg-white hover:text-black transition-all">
                          Documentation <ExternalLink className="w-4 h-4"/>
                        </a>
                        <a href={item.videoLink} target="_blank" rel="noreferrer" className="w-full sm:w-auto flex-1 px-6 py-4 bg-red-500/5 border border-red-500/20 rounded-xl text-[10px] font-black uppercase text-red-400 text-center flex items-center justify-center gap-2 hover:bg-red-500 hover:text-white transition-all">
