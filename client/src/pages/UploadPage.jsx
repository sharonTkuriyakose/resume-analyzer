...
const UploadPage = ({ onAnalysisComplete }) => {
  ...
  return (
    <div className="min-h-screen w-full bg-[#0A0C10] text-slate-200 font-sans selection:bg-indigo-500/30 overflow-x-hidden">
      {/* GLOBAL CSS RESET */}
      ...
      <div className="max-w-[1700px] mx-auto px-6 md:px-12 space-y-24 md:space-y-48 pt-8 md:pt-12">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center pb-6 md:pb-8 border-b border-white/5 gap-4">
            <div className="flex items-center gap-4 text-white font-black text-sm md:text-xl uppercase tracking-[0.2em] md:tracking-[0.4em] text-center sm:text-left">
                <Activity className="w-5 h-5 text-indigo-500 shrink-0" /> System Neural Link Active
            </div>
        ...
        {/* 2. HERO SECTION - Single Column on Mobile */}
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24 items-center">
          <div className="lg:col-span-7 space-y-8 md:space-y-14 text-center lg:text-left">
            ...
            <h1 className="text-5xl sm:text-7xl lg:text-9xl font-black text-white leading-[1.1] lg:leading-[0.9] tracking-tighter">
              Elevate Your <br />
            ...
            <div className="space-y-6 md:space-y-8">
                ...
                <div className="flex flex-wrap justify-center lg:justify-start gap-4 md:gap-8 pt-4">
                    <div className="flex items-center gap-3 text-slate-300 font-bold text-[10px] md:text-sm uppercase tracking-widest bg-white/5 px-4 py-2 rounded-lg border border-white/5">
            ...
            {/* Proof of Concept Avatars - Scaled for Mobile */}
            <div className="flex flex-col sm:flex-row items-center gap-6 md:gap-12 pt-6 md:pt-10">
              <div className="flex -space-x-4 md:-space-x-6">
                {PEOPLE.map((url, i) => (
                  <div key={i} className="w-12 h-12 md:w-20 md:h-20 rounded-full border-[4px] md:border-[8px] border-[#0A0C10] shadow-2xl overflow-hidden">
                    <img src={url} alt="User" className="w-full h-full object-cover grayscale" />
                  </div>
                ))}
                <div className="w-12 h-12 md:w-20 md:h-20 rounded-full border-[4px] md:border-[8px] border-[#0A0C10] bg-indigo-600 flex items-center justify-center text-white text-[10px] md:text-sm font-black">
                  +500
                </div>
              </div>
          ...
          {/* UPLOAD CARD - Adjusted for smaller Viewports */}
          <div className="lg:col-span-5 relative py-12 lg:py-0">
            ...
            <div className="bg-white/[0.03] backdrop-blur-3xl p-8 md:p-16 rounded-[2.5rem] md:rounded-[4.5rem] border border-white/10 shadow-2xl relative">
        ...
        {/* 3. FEATURE BENTO GRID - Responsive Columns */}
        <section className="space-y-12 md:space-y-24">
          <div className="flex flex-col items-center text-center space-y-4 md:space-y-8">
            ...
            <h2 className="text-4xl md:text-7xl font-black text-white tracking-tight uppercase">Full-Stack Intelligence</h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
            {[
              ...
              <div key={i} className="group bg-white/[0.02] p-8 md:p-14 rounded-[2.5rem] md:rounded-[4rem] border border-white/5 hover:border-indigo-500/30 transition-all duration-700">
                <div className={`w-16 h-16 md:w-24 md:h-24 bg-white/5 ${feature.color} rounded-2xl md:rounded-3xl flex items-center justify-center mb-8 md:mb-12 group-hover:scale-110 transition-transform shadow-2xl`}>
                  {feature.icon}
                </div>
        ...
        {/* 4. SYSTEM STATS - Stacks on Mobile */}
        <section className="bg-white/[0.03] border border-white/5 rounded-[3rem] md:rounded-[6rem] p-12 md:p-28 text-white grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-12 md:gap-20 text-center relative overflow-hidden shadow-2xl mb-12 md:mb-24">
          ...
          {[
            ...
            <div key={i} className="space-y-4 md:space-y-8 relative z-10">
              <div className="flex items-center justify-center gap-3 text-indigo-400/60">
                 {stat.icon}
                 <span className="text-[10px] md:text-base font-black uppercase tracking-[0.2em] md:tracking-[0.3em]">{stat.label}</span>
              </div>
              <h4 className="text-5xl md:text-8xl font-black tracking-tighter text-white">{stat.val}</h4>
            </div>
          ))}
        </section>
